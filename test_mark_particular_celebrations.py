...
def test_mark_particular_celebrations():
    ...
    national_calendar_usa = {
        "litcal": [
            ...
            {"event_key": "Christmas", "name": "The Nativity of the Lord"},  # In general calendar
            {"event_key": "ImmaculateConceptionUSA", "name": "Immaculate Conception"},  # Particular to USA
            {"event_key": "StPatrick", "name": "[ USA ] Saint Patrick"},  # Particular with brackets
            {"event_key": "OurLadyOfGuadalupe", "name": "Our Lady of Guadalupe"},  # Particular without brackets
        ]
    }
    ...
    national_calendar_italy = {
        "litcal": [
            {"event_key": "Easter", "name": "Easter Sunday"},  # In general calendar
            {"event_key": "AllSaints", "name": "All Saints' Day"},  # In general calendar
            {"event_key": "StFrancisOfAssisi", "name": "San Francesco d'Assisi"},  # Particular, no brackets
            {"event_key": "StCatherineOfSiena", "name": "Santa Caterina da Siena, Patrona d'Italia"},  # Particular, modified name
        ]
    }
    ...
    # Verify USA results
    assert result_usa["litcal"][0]["is_particular"] == False, "Easter should not be particular"
    assert result_usa["litcal"][1]["is_particular"] == False, "Christmas should not be particular"
    assert result_usa["litcal"][2]["is_particular"] == True, "Immaculate Conception USA should be particular"
    assert result_usa["litcal"][3]["is_particular"] == True, "St Patrick USA should be particular"
    assert result_usa["litcal"][4]["is_particular"] == True, "Our Lady of Guadalupe should be particular"
    ...
    print("="*60)
    result_italy = mark_particular_celebrations(national_calendar_italy, general_calendar)
    ...
    assert result_italy["litcal"][0]["is_particular"] == False, "Easter should not be particular"
    assert result_italy["litcal"][1]["is_particular"] == False, "All Saints should not be particular"
    assert result_italy["litcal"][2]["is_particular"] == True, "St Francis should be particular"
    assert result_italy["litcal"][3]["is_particular"] == True, "St Catherine should be particular"
